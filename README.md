...
山札の計算そのものは決定的（入力が同じなら同じ山札）ですが、入力のシードは毎ハンド新しく作ります。

- サーバーシード: 各ハンドのベット受付開始時に `newServerSeed()`（`crypto.getRandomValues` で 32 バイト）を毎回呼ぶ（`room.ts` の `newFair`）。卓を作り直しても、同じハンド番号でも別の値になる。
- クライアントシード: 各プレイヤーのブラウザがハンドごとに `crypto.getRandomValues` で作る（`fairSeeds.ts`）。
- drand: ハンドごとに違うラウンドの値。

コードの中で固定に見えるシードは、次のどちらかです（本番の卓では使われない）。

- テスト（`*.test.ts`）の `test-seed-N` などの固定値: 結果を毎回同じにして検算するため。サーバーのテスト `room.test.ts` には「卓を作り直して同じ人・同じシードで 1 ハンド目を始めても、20 回とも山札が違う」ことの確認もある。
- 10 万ハンドの統計レポート（`scripts/fairness/report.ts`）: 1 回の実行の中では各ハンドのシードを「マスターシード（実行ごとに暗号乱数で作る）+ ハンド番号」から作る。あとから全ハンドを再計算して確かめられるようにするため。
